...
namespace Netologia.Behaviours
{
		...
		where TRef : Component
	{
		...
		/// Если пул ещё не создан, создает его.
		/// </summary>
		...
		public GameObjectPool<TRef> this[TRef prefab]
			=> _pools.TryGetValue(prefab, out var pool)
				? pool
				: CreatePool(prefab, _poolCapacity);

		/// <summary>
		/// Создан ли пул объектов для указанного префаба.
		/// </summary>
		/// <param name="prefab">Источник пула.</param>
		/// <returns>True, если пул уже создан.</returns>
		...
		public GameObjectPool<TRef> CreatePool(TRef prefab, int capacity)
		{
			...
			{
#if UNITY_EDITOR
				throw LogUtility.Throw(
					nameof(GameObjectPoolContainer<TRef>),
					nameof(CreatePool),
